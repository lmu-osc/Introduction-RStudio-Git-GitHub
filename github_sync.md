...
The **Quick Setup** screen gives sets of git commands that can be used in various circumstances. We are interested in **…or push an existing repository from the command line**. Copy these commands to the clipboard using the copy button on GitHub. Note that 'master' (in the screenshots) is the historical name of the default branch (the default branch is currently the only branch in this repository and branches are diverging versions of a file **within** a repository, but we will not cover this concept today). The default branch is now called ‘main’ as part of the company’s effort to remove unnecessary references to slavery.
